...
void  ServoDecodeClass::setFailsafe(){
    // setFailsafe with no arguments sets failsafe for all channels to their current value
...
int ServoDecodeClass::GetChannelPulseWidth( uint8_t channel)
{
    ...
    if( channel <=  MAX_CHANNELS)  {
        if( (State == FAILSAFE_state || State == NOT_SYNCHED_state || State == ACQUIRING_state)&& (Failsafe[channel] > 0 ) )
